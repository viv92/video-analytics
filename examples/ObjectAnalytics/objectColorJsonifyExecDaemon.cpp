                ...
                if (frameNumber % (FPS / procFPS) == 0) {
                    ...
                    for(int i = 0; i < objectAnalytics.blobContainer.size(); i++) {
                        if (objectAnalytics.blobContainer[i].lastFrameNumber == frameNumber) {
                            ...
                            objectAnalytics.blobContainer[i].metaData = color + " " + objectAnalytics.blobContainer[i].label;
                        }
...
int main( int argc, char *argv[]) {
    if ( argc < 2 ) {
        std::cout << "Usage: ./objectColorJsonifyExecDaemon [aiSaacSettingsPath] [port]";
        return -1;
    }
    ...
    colorClassifier = aiSaac::ColorClassifier();
