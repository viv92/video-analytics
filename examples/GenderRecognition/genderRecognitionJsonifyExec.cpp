...
int main(int argc, char *argv[]) {
    try {
        if ( argc < 4 ) {
            std::cout << "Usage: ./genderRecognitionJsonifyExec [file] [aiSaacSettingsPath] [mongoResultTableID]" << std::endl;
            return -1;
        }
            ...
            while (true) {
                ...
                if (frameNumber % (FPS / procFPS) == 0) {
                    ...
                    for (int i = 0; i < faceAnalytics.blobContainer.size(); i++) {
                        faceAnalytics.blobContainer[i].gender = genderRecognition.detect(currentFrame);
                    }
