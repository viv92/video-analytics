...
void initiateDaemonController(std::string storageFolderPath, std::string aiSaacSettingsPath, std::string encoderCameraSettingsPath) {
    ...
    ruleAlert rtRuleAlert[ruleCount];
    ...
    folderCreationCommand = "mkdir -p " + outputFolderPath + " " + debugFolderPath;
    ...
    while (true) {
        currentFrame = rtspStreamer->getFrame();
        cv::rectangle(currentFrame, cv::Point(100, 100), cv::Point(200, 200), cv::Scalar(0, 0, 255));
        frameNumber++;
        ...
        if (frameNumber % (FPS / procFPS) == 0) {
            ...
            if (bgSubAnalytics->detect(motionDetectFrame, frameNumber)) {
                ...
                if (objectAnalyticsEnabledFlag) {
                    objectAnalytics->track(motionDetectFrame, frameNumber, procFrameNumber);
                    if (objectAlertEnabled) {
                        for (int i = 0; i < objectAnalytics->blobContainer.size(); i++) {
...
int main(int argc, char const *argv[]) {
    if ( argc < 4 ) {
        std::cout << "Usage: ./daemonControllerEncoderNoClientObject [storageFolderPath] [aiSaacSettingsPath] [encoderCameraSettingsPath]" << std::endl;
        return -1;
    }
