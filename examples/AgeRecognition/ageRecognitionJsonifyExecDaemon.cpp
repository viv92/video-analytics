...
int performAgeRecognition(std::string file, std::string aiSaacSettingsPath, std::string resultTableId, socket_ptr sock) {
    ...
    aiSaac::AgeRecognition ageRecognition = aiSaac::AgeRecognition(aiSaacSettings, *caffeClassifier);
                ...
                if (frameNumber % (FPS / procFPS) == 0) {
                    ...
                    for (int i = 0; i < faceAnalytics.blobContainer.size(); i++) {
                        if (faceAnalytics.blobContainer[i].lastFrameNumber == frameNumber) {
                            faceAnalytics.blobContainer[i].age =
                                ageRecognition.detect(
...
void session(socket_ptr sock) {
    try {
        ...
        std::string log_message = "Age recognition finished";
        memset(data, '\0', max_length);
...
int main( int argc, char *argv[]) {
    if ( argc < 3 ) {
        std::cout << "Usage: ./ageRecognitionJsonifyExecDaemon [aiSaacSettingsPath] [port]";
        return -1;
    }
    ...
    caffeClassifier = new aiSaac::CaffeClassifier(
        globalAiSaacSettings.getAgePrototxtPath(),
        globalAiSaacSettings.getAgeCaffemodelPath(),
        globalAiSaacSettings.getAgeMeanFilePath(),
        globalAiSaacSettings.getAgeLabelFilePath());
