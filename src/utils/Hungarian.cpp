...
{
	std::cout << "in hungarian solve" << std::endl << std::flush;
	...
	std::cout << "fill distance matrix" << std::endl << std::flush;
	for (unsigned int i = 0; i < nRows; i++)
		for (unsigned int j = 0; j < nCols; j++)
			distMatrixIn[i + nRows * j] = DistMatrix[i][j];

	// call solving function
	std::cout << "call solving function" << std::endl << std::flush;
	assignmentoptimal(assignment, &cost, distMatrixIn, nRows, nCols);

	Assignment.clear();
	std::cout << "populate assignments" << std::endl << std::flush;
