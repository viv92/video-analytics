...
vector<struct DenseFeature> readFeatureVectorsDense(string featureVectorPath, int&n, int& numFeatures)
{
  ifstream file;
  file.open(featureVectorPath, ios::in | ios::out);
  cout << "Starting to read the file\n";
      if(!file.is_open()) {
          return vector<struct DenseFeature>();
      }
      vector<struct DenseFeature> featureVectors;
      string line;
      int index = 0;
      while(!std::getline(file, line, '\n').eof()) {
          ...
          DenseFeature d;
          d.index = index;
          d.featureVec = lineData;
          numFeatures = lineData.size();
          d.numFeatures = numFeatures;
          index++;
          featureVectors.push_back(d);
      }
      n = featureVectors.size();
      cout << "Done reading the file. Number of examples is " << n << " and the number of features is " << numFeatures << "\n";
