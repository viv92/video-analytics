 ...
 *	L2 Regularized Logistic Regression (with both two class and multi-class classification)
    ...
    algtype: type of algorithm:
		...
		Note: Only the default method (i.e. 0 is guaranteed to work in this case)
 *
*/
...
#include "L2HingeSVM.h"
#include "../../optimization/contAlgorithms/contAlgorithms.h"
#include "../../optimization/contFunctions/L2HingeSVMLoss.h"
...
namespace datk {
	...
	L2HingeSVM<Feature>::L2HingeSVM(vector<Feature>& trainFeatures, Vector& y, int m, int n, int nClasses,
	...
	template <class Feature>
	L2HingeSVM<Feature>::L2HingeSVM(const L2HingeSVM<Feature>& c) : Classifiers<Feature>(c.m, c.n),
	...
	template <class Feature>
	L2HingeSVM<Feature>::~L2HingeSVM(){}

	template <class Feature>
	void L2HingeSVM<Feature>::train(){ // train L2 regularized logistic regression
		if (nClasses == 2){
			trainOne(y, w);
		}
		else{
	...
	template <class Feature>
	void L2HingeSVM<Feature>::trainOne(Vector& yOne, Vector& wcurr){
	L2HingeSVMLoss<Feature> ll(m, trainFeatures, yOne, lambda);
	if (algtype == 0){
		//cout<<"*******************************************************************\n";
		//cout<<"Training using Dual Coordinate Descent Algorithm...\n";
		wcurr = SVCDual(trainFeatures, y, 2, 1, 1e-3, maxIter, 0);
	}
	else if (algtype == 1){
		cout<<"*******************************************************************\n";
	...
	// save the model
	template <class Feature>
	int L2HingeSVM<Feature>::saveModel(const char* model){
	...
	template <class Feature>
	int L2HingeSVM<Feature>::saveJsonModel(const char* model){
	...
	template <class Feature>
	int L2HingeSVM<Feature>::loadModel(const char* model){
	...
	template <class Feature>
	int L2HingeSVM<Feature>::loadJsonModel(const char* model){
	...
	template <class Feature>
	double L2HingeSVM<Feature>::predict(const Feature& testFeature, double& val){
	...
	template <class Feature>
	double L2HingeSVM<Feature>::predict(const Feature& testFeature){
		// the assumption here is that train and test datasets have the same number of features
		double val;
		if (nClasses == 2){
			val = featureProductCheck(w, testFeature);
	...
	// classification.
	template <class Feature>
	void L2HingeSVM<Feature>::predictProbability(const Feature& testFeature, Vector& prob){
		// the assumption here is that train and test datasets have the same number of features
  ...
  template class L2HingeSVM<SparseFeature>;
}
