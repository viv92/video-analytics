...
#ifndef L2_LOGISTIC_REGRESSION_H
#define L2_LOGISTIC_REGRESSION_H
...
namespace datk {
  ...
  template <class Feature>
	class L2LogisticRegression: public Classifiers<Feature>{
	protected:
	...
	public:
		L2LogisticRegression(vector<Feature>& trainFeatures, Vector& y, int m, int n, int nClasses = 2, double lambda = 1,
		int algtype = 0, int maxIter = 250, double eps = 1e-2, int miniBatch = 100, int lbfgsMemory = 100, int verbosity = 0);
        L2LogisticRegression(const L2LogisticRegression& c); // copy constructor
		...
		int loadModel(const char* model); // load the model
