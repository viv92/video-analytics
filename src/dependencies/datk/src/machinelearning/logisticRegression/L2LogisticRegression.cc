...
#include "L2LogisticRegression.h"
...
namespace datk {
	...
	L2LogisticRegression<Feature>::L2LogisticRegression(vector<Feature>& trainFeatures, Vector& y, int m, int n, int nClasses,
		double lambda, int algtype, int maxIter, double eps, int miniBatch, int lbfgsMemory, int verbosity): Classifiers<Feature>(m, n), nClasses(nClasses),
		trainFeatures(trainFeatures), y(y),
	lambda(lambda), algtype(algtype), maxIter(maxIter), eps(eps), miniBatch(miniBatch), lbfgsMemory(lbfgsMemory), verbosity(verbosity) {}

	template <class Feature>
	L2LogisticRegression<Feature>::L2LogisticRegression(const L2LogisticRegression<Feature>& c) : Classifiers<Feature>(m, n),
	trainFeatures(c.trainFeatures), y(c.y), lambda(c.lambda), algtype(c.algtype), nClasses(c.nClasses),
	maxIter(c.maxIter), eps(c.eps), miniBatch(c.miniBatch), lbfgsMemory(c.lbfgsMemory), verbosity(c.verbosity) {}

	template <class Feature>
	L2LogisticRegression<Feature>::~L2LogisticRegression(){}

	template <class Feature>
	void L2LogisticRegression<Feature>::train(){ // train L2 regularized logistic regression
		if (nClasses == 2){
			trainOne(y, w);
		}
		else{
	...
	template <class Feature>
	void L2LogisticRegression<Feature>::trainOne(Vector& yOne, Vector& wcurr){
		// cout << trainFeatures.size() << " " << yOne.size() << " " << m << " " << trainFeatures[0].numFeatures << "\n";
	L2LogisticLoss<Feature> ll(m, trainFeatures, yOne, lambda);
	if (algtype == 0){
		// cout<<"*******************************************************************\n";
		// cout<<"Training using Trust Region Newton Algorithm...\n";
		wcurr = tron(ll, Vector(m, 0), maxIter, eps, verbosity);
	}
	else if (algtype == 1){
		cout<<"*******************************************************************\n";
	...
	// save the model
	template <class Feature>
	int L2LogisticRegression<Feature>::saveModel(const char* model){
	...
	template <class Feature>
	int L2LogisticRegression<Feature>::saveJsonModel(const char* model){
	...
	template <class Feature>
	int L2LogisticRegression<Feature>::loadModel(const char* model){
	...
	template <class Feature>
	int L2LogisticRegression<Feature>::loadJsonModel(const char* model){
	...
	template <class Feature>
	double L2LogisticRegression<Feature>::predict(const Feature& testFeature, double& val){
		...
		if (nClasses == 2){
			...
			for (int j = 0; j < nClasses; j++){
				assert(testFeature.numFeatures == wMany[j].size());
	...
	template <class Feature>
	double L2LogisticRegression<Feature>::predict(const Feature& testFeature){
		// the assumption here is that train and test datasets have the same number of features
		double val;
		if (nClasses == 2){
                    	val = featureProductCheck(w, testFeature);
	...
	// classification.
	template <class Feature>
	void L2LogisticRegression<Feature>::predictProbability(const Feature& testFeature, Vector& prob){
		// the assumption here is that train and test datasets have the same number of features
		...
		if (nClasses == 2){
			...
			double sum = 0;
			for (int j = 0; j < nClasses; j++){
                //codeReview(Khoshrav): Active Learning-Transfer Learning. Why is val = 0
				val = featureProductCheck(wMany[j], testFeature);
				prob[j] = 1/(1 + exp(-val));
                //codeReview(Khoshrav): use prob[j] = exp(val) instead
                // prob[j] = exp(val);
                //codeReview(Khoshrav): predict() uses argMax on wMany[j]*testFeature
				sum += prob[j];
			}
			for (int j = 0; j < nClasses; j++)
				prob[j] = prob[j]/sum;
  ...
  template class L2LogisticRegression<SparseFeature>;
  template class L2LogisticRegression<DenseFeature>;
