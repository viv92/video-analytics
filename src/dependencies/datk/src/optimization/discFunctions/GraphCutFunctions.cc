...
namespace datk {
    ...
    GraphCutFunctions::GraphCutFunctions(int n, const vector<vector <float> >& kernel, const double lambda) : SubmodularFunctions(n), kernel(kernel), lambda(lambda), sizepreCompute(n){
...
GraphCutFunctions::GraphCutFunctions(const GraphCutFunctions& f) : SubmodularFunctions(f.n), kernel(f.kernel), lambda(f.lambda), sizepreCompute(f.n){
...
/*
GraphCutFunctions& GraphCutFunctions::operator=(const GraphCutFunctions & f){
    if (this != &f){
        n = f.n;
        kernel = f.kernel; 
        lambda = f.lambda;
