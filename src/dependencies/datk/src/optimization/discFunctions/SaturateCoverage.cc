...
namespace datk {
    SaturateCoverage::SaturateCoverage(int n, const vector<vector <float> >& kernel, const double alpha) : SubmodularFunctions(n), kernel(kernel), alpha(alpha), sizepreCompute(n){	
        preCompute.assign(sizepreCompute, 0); //precomputed statistics for speeding up greedy
        if (alpha <= 0){
            error("Cannot input alpha = %f for the saturated coverage function to be smaller or equal to 0\n", alpha);
        }
    ...
    SaturateCoverage::SaturateCoverage(const SaturateCoverage& f): SubmodularFunctions(f), kernel(f.kernel), alpha(f.alpha), sizepreCompute(f.sizepreCompute){
